...
class TestInversaoValores:
    ...
    def test_inverter_valor_escala_5(self, scoring_service):
        ...
        assert scoring_service.inverter_valor(1, escala_max=5) == 4
        assert scoring_service.inverter_valor(2, escala_max=5) == 3
        assert scoring_service.inverter_valor(3, escala_max=5) == 2
        assert scoring_service.inverter_valor(4, escala_max=5) == 1
        assert scoring_service.inverter_valor(5, escala_max=5) == 0
...
class TestCalculoMedia:
    ...
    def test_media_com_inversao(self, scoring_service):
        ...
        respostas = [
            ...
            {"id_pergunta": "VLT_CV_03", "valor": 5},  # Invertido: 5→0
        ]
        media = scoring_service.calcular_media_dimensao(
            respostas=respostas,
            codigo_questionario="COPSOQ_MEDIA_PT"
        )
        assert media == 2.0  # (4+0)/2 = 2.0
